...
//
// Ported from shell on 2026-08-17, 124 ms → 52 ms: it forked `grep` + `sed` once per note, up to
// 45 subprocesses to print 15 lines. Pinning the vault to local disk moved it 174.0 → 165.2 ms,
// which is what identified it as fork-bound rather than I/O-bound, and the port fixed a latent
// `set -e` bug that silently dropped every bullet — docs/decisions/2026-08-17-shell-vs-node-hooks.md.
