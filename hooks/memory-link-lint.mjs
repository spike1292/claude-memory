...
function main() {
  ...
  // One pass over the corpus. The shell did this once per note.
  const corpus = [];
  ...
  if (orphans.length) {
    console.log('MOC-only memory notes (in MEMORY.md but no inbound [[wikilink]] from any sibling — the note');
    console.log('graph cannot reach them). Link each from a note someone would be reading when they need it:');
    for (const n of orphans) console.log(`  - ${n}`);
  }

  const moc = path.join(mem, 'MEMORY.md');
  const byName = new Map(corpus.map(([f, t]) => [f, t]));
  if (!byName.has(moc)) return;
  ...
  if (drift.length) {
    console.log('');
    console.log('MEMORY.md figure drift — a bolded number in the MOC hook is absent from the note it points');
    console.log('at. Usually the note was corrected and the hook was not. Check the note, then fix the hook:');
    for (const d of drift) console.log(`  - [[${d.target}]] hook says ${d.n}, not found in the note`);
  }
}
...
if (isMain && process.argv.includes('--selftest')) {
  ...
  const read = (t) => (t in notes ? notes[t] : null);
  ...
  console.log('selftest: 17 assertions passed');
} else if (isMain) {
