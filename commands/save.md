...
1. Slug: `<slug>` = the project key: normalised git remote of `pwd` (e.g. `gitlab.essent.nl-sitecoreplus-frontend`), **NOT the checkout path** — run `. ~/.claude/hooks/lib/vault-env.sh; project_key "$PWD"`. Vault root: `<vault>` = `. ~/.claude/hooks/lib/vault-env.sh; resolve_vault`.
   **Never use `$CLAUDE_VAULT` directly.** It is **unset** on any machine using the legacy-path fallback (this one), so `$CLAUDE_VAULT/Logs/…` expands to `/Logs/…` and writes outside the vault or fails at `/`. Every hook calls `resolve_vault`; commands must too.
