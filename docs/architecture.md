...
```
  ...
  │  logs/            free-form logs (1 MB cap), dated JSONL (age cap)    │
...
| R5 | **Unbounded `$CLAUDE_MEMORY_HOME`** — **halved 2026-08-19 (#24), `logs/` closed 2026-08-21 (#53)** | growth is unchanged for `db/`, `models/` and `eval/` (no `VACUUM`; per-project × per-model `.db` files accumulate; `models/` accumulates per model; `prune-logs.mjs` touches only the vault) but it is no longer unobserved: `doctor.sh` reports the size of `$STATE` and warns past 2 GB, and the hook logs — the one component that grew without bound — are capped at 1 MB in `logBanner()`/`openLog()`, and the dated JSONL families are deleted past `logRetentionDays()` (30) by `pruneDatedLogs()` on the first append of a new day (#53). `doctor.sh --perf` (#38) now itemises the growth rather than only totalling it: the per-directory split, and one row per index with its size, chunks and whether its model is still the active one — an index on an inactive model is the accumulation this row describes, and the report names it | bounded logs; the rest is multi-GB but now **noticed early**, and attributable |
...
| R8 | **Symlink-dereference bugs** | the class, not an instance: two found so far (`du` without `-L`; the runtime probe in `semantic-index-refresh`). Both fixed, the second now pinned by a test that symlinks `node_modules` and asserts `runtimeInstalled()` sees through it. Every new check against `$CLAUDE_MEMORY_HOME` is another draw — `dirUsage()` in `doctor-perf.mjs` is the third, and skips symlinks rather than dereferencing them; none of the six directories it scans is a link today, so that is a standing precaution against this row, not a fix for a known instance | a check that measures nothing |
