...
if (process.argv.includes('--deferred')) {
  ...
  if (!fs.existsSync(reflections))
    console.log(`⚠ no REFLECTIONS.md for this project — wrong directory?`);
  if (!stillDeferred.length) console.log('nothing still deferred in REFLECTIONS.md');
  else {
    console.log(
      `${stillDeferred.length} item(s) still deferred — handle these BEFORE new findings:`,
    );
...
for (const f of mds(MEM)) {
  ...
  notes.set(f.slice(0, -3), {
    file: path.join(MEM, f),
    layer: 'Memory',
    body: read(path.join(MEM, f)),
  });
}
for (const fo of FOLDERS)
  for (const f of mds(path.join(INS, fo)))
    notes.set(f.slice(0, -3), {
      file: path.join(INS, fo, f),
      layer: fo,
      body: read(path.join(INS, fo, f)),
    });
...
console.log(`# ${SLUG}`);
console.log(
  `snapshot: ${memNames.length} Memory + ${notes.size - memNames.length} Insights (${counts})`,
);
console.log(
  'NOTE: the distiller writes concurrently — re-run at the end and report both counts if they moved.',
);
...
section(
  ...
  memNames
    .filter((n) => inbound.get(n) === 0)
    .map((n) => `${n} — link it from a note someone reads before needing it`),
);
...
for (const n of memNames) {
  ...
  if (declared && declared.trim().replace(/^["']|["']$/g, '') !== n)
    nameMismatch.push(`${n} — frontmatter says ${declared.trim()}`);
}
...
for (const n of memNames) {
  ...
  for (const line of body.split('\n')) {
    for (const m of line.matchAll(claimRe)) {
      ...
      if (full)
        suffixOnly.push(
          `${n}: \`${c}\` → real path is \`${full[0]}\`${full.length > 1 ? ` (+${full.length - 1} more)` : ''}`,
        );
      else absent.push(`${n}: ${c}`);
    }
  }
}
section('Path abbreviated to a suffix — WRITE THE FULL PATH', [...new Set(suffixOnly)]);
section(
...
for (const fo of FOLDERS) {
  const docs = mds(path.join(INS, fo)).map((f) => ({
    f: f.slice(0, -3),
    t: toks(read(path.join(INS, fo, f))),
  }));
...
if (dups.length)
  console.log(
    '  → hand these to /memory:prune in the SAME session; a deferred pair only survives if the next prune happens to rank it.',
  );
...
for (const [n, v] of notes) {
  ...
  if (s.declared) {
    (resolves(s.declared) ? supersededOk : danglingSupersede).push(
      `${n} → ${s.declared}${s.on ? ` (${s.on})` : ' — no superseded_on:'}`,
    );
  } else if (s.inProse && !s.marked && v.layer === 'Memory') {
    proseOnly.push(
      `${n} — reversal stated in prose only; mark the claim inline: (superseded YYYY-MM-DD by [[note]])`,
    );
  }
}
...
for (const [n, v] of notes) {
  ...
  lines.forEach((l, i) => {
    ...
    if (isUnprovenancedMetric(l, ctx))
      unprovenanced.push(`${n}:${i + 1} ${l.trim().slice(0, 105)}`);
  });
}
section(
  ...
  unprovenanced
    .slice(0, 12)
    .concat(unprovenanced.length > 12 ? [`… +${unprovenanced.length - 12} more`] : []),
);
